...
def load_backdoored_model(backdoor_method, clean_model_path, backdoored_model_path, lora_weights_path, device='cuda'):
    if backdoor_method == 'ed':
        ...
        pipe = pipe.to(device)
        state_dict = torch.load(backdoored_model_path, map_location=device)
        if "embeddings.position_ids" in state_dict:

            del state_dict["embeddings.position_ids"]
        pipe.text_encoder.text_model.load_state_dict(state_dict)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backdoored_model_path', type=str,default='/ai/data/ReFACT/images/results/RoAD/validation/contrastive_text/use_kl_prompts_False_kl_factor_0/use_negative_images_True_20/num_editing_images_1/v_prob_threshold_0.99/v_max_grad_steps_100/v_lr_0.05/similarity_metric_l2/edit_layer_7/clip_text_model_edited.pth')
